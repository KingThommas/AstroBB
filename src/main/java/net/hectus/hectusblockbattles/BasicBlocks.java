...
public class BasicBlocks {
    private final Material[] groupBlocks = {Material.NETHER_WART_BLOCK, Material.NETHERRACK, Material.NETHER_GOLD_ORE, Material.NETHER_BRICKS, Material.RED_NETHER_BRICKS, Material.NETHERITE_BLOCK, //the first block of a group has a special ability, here it is removing opponents luck boost
                                            Material.SEA_LANTERN, Material.PRISMARINE, Material.PRISMARINE_BRICKS, Material.DARK_PRISMARINE, Material.WET_SPONGE, null, //tells the player who's winning
    ...
    public double calculateGameScore(double currentGameScore, Material material, Material lastBlock, boolean turn, double[] blockBoosts) {
        double power = -1;
        int group = -1;
        int counterGroup = -1;
    ...
    public boolean blockCheck(Material material, String warp, boolean night) {
        int group = -1;
        for (int i = 0; i < groupBlocks.length; i++) {
            if(groupBlocks[i]==material) {
                group = (int) Math.floor((i+1)/6);
